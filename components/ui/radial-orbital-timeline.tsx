...
export default function RadialOrbitalTimeline({ timelineData }: RadialOrbitalTimelineProps) {
  ...
  const isRelated = (itemId: number) => activeNodeId ? getRelatedItems(activeNodeId).includes(itemId) : false;
  ...
  return (
    <div className="w-full h-[600px] flex flex-col items-center justify-center bg-black rounded-2xl overflow-hidden" ref={containerRef} onClick={handleContainerClick}>
      <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
        <div className="absolute w-full h-full flex items-center justify-center" ref={orbitRef} style={{ perspective: "1000px" }}>
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              <div key={item.id} ref={(el) => { nodeRefs.current[item.id] = el; }} className="absolute transition-all duration-700 cursor-pointer" style={{ transform: `translate(${pos.x}px, ${pos.y}px)`, zIndex: expanded ? 200 : pos.zIndex, opacity: expanded ? 1 : pos.opacity }} onClick={(e) => { e.stopPropagation(); toggleItem(item.id); }}>
                <div className={`absolute rounded-full -inset-1 ${pulsing ? "animate-pulse" : ""}`} style={{ background: "radial-gradient(circle, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0) 70%)", width: `${item.energy * 0.5 + 40}px`, height: `${item.energy * 0.5 + 40}px`, left: `-${(item.energy * 0.5) / 2}px`, top: `-${(item.energy * 0.5) / 2}px` }} />
                <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${expanded ? "bg-white text-black border-white shadow-lg shadow-white/30 scale-150" : related ? "bg-white/50 text-black border-white animate-pulse" : "bg-black text-white border-white/40"}`}>
                  <Icon size={16} />
                </div>
                <div className={`absolute top-12 whitespace-nowrap text-xs font-semibold tracking-wider transition-all duration-300 ${expanded ? "text-white scale-125" : "text-white/70"}`}>{item.title}</div>
                {expanded && (
                  <Card className="absolute top-20 left-1/2 -translate-x-1/2 w-64 bg-black/90 backdrop-blur-lg border-white/30 shadow-xl shadow-white/10 overflow-visible">
                    ...
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-center">
                        <Badge className={`px-2 text-xs ${statusStyle(item.status)}`}>{item.status === "completed" ? "COMPLETE" : item.status === "in-progress" ? "IN PROGRESS" : "PENDING"}</Badge>
                        ...
                        <div className="mt-4 pt-3 border-t border-white/10">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {item.relatedIds.map((rid) => {
                              ...
                              return (<Button key={rid} variant="outline" size="sm" className="h-6 px-2 py-0 text-xs border-white/20 bg-transparent hover:bg-white/10 text-white/80" onClick={(e) => { e.stopPropagation(); toggleItem(rid); }}>{ri?.title}<ArrowRight size={8} className="ml-1" /></Button>);
                            })}
                          </div>
                        </div>
                      )}
